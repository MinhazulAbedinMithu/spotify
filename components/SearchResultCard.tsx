...
const SearchResultCard = ({ data }: any) => {
  ...
  const getArtist = (id: string) => {
    ...
    axios
      .get("https://lazy-puce-bandicoot-wig.cyclic.app/api/search/artist", {
        params: {
          query: id,
        },
      })
  ...
  return (
    <div className="w-[300px] flex flex-col relative text-white">
      ...
      <div className="p-1 absolute bottom-0 w-full">
        <h2 className="font-semibold bg-black text-[14px] w-fit rounded p-1">
          {data?.name?.slice(0, 33)}
          {data?.name?.length > 33 && "..."}
        </h2>
        <div className="flex items-center flex-wrap mt-1 gap-2">
          {data?.artists?.map(({ name, id }: any, index: number) => (
            <button
              onClick={() => getArtist(id)}
              className="bg-spotify text-[12px] p-1 rounded"
              key={index}
            >
              {name}dsfsdf
            </button>
